...
# StatMatrixAxes$compute_panel(
#  data = tibble(xc = -1, yc = 0, lx = 1, ly = 2, lz = 3), scales = NULL,
#  degx = 70, degy = 20, degz = 0,
#  viewx = 2, viewy = 2, respect = TRUE
# ) %>%
#  ggplot() +
...
StatMatrixAxes = ggplot2::ggproto(

  `_class` = 'StatMatrixAxes',
  `_inherit` = Stat,
      ...
      dplyr::group_modify(
        ~(function(dotx) {
          tidyr::crossing(
            dotx,
            # pts_uni
            pts_matrix_axes(lx = dotx$lx[1], ly = dotx$ly[1], lz = dotx$lz[1])
          ) %>%
