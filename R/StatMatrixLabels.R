...
#'StatMatrixLabels$compute_panel(
...
#'  geom_matrix(data = tibble(), mapping = aes(xc = 0, yc = 0, lx = 1, ly = 2, lz = 3), viewx = 2, viewy = 2, respect = TRUE) +
#'  geom_text(mapping = aes(x = x, y = y, label = label, angle = angle)) +
#'  geom_rect(data = tibble(), mapping = aes(xmin = -1, xmax = 1, ymin = -1, ymax = 1), fill = NA, colour = 'black')

StatMatrixLabels = ggplot2::ggproto(

  `_class` = 'StatMatrixLabels',
  `_inherit` = Stat,
    ...
    data %>%
      ...
      dplyr::group_modify(
        ~(function(dotx) {
          pts = NULL
          if (combined) {
            pts = pts_matrix_combined_labels(lx = dotx$lx[1], ly = dotx$ly[1], lz = dotx$lz[1])
          } else {
            pts_matrix_labels(lx = dotx$lx[1], ly = dotx$ly[1], lz = dotx$lz[1])
          }
